...
* Be simple: If the implementation is hard to explain, it's a bad idea. If the
...
* Be reasonable: `A foolish consistency is the hobgoblin of small minds <>`__.
  Within reason. If style interferes with the *functionality* of code, style
  must give way. And always remember, we are here to get stuff done. 
...
*Rationale: I shouldn't have to scroll across to see that the line ends "unless
...". Also, lines that are longer than 80 characters are a sign that 1. you're
probably trying to do too much in one line or 2. your variable names are far too
long.*
...
* No spaces after (, [ and before ], ). except where doing multiline lists or
  hashes.
